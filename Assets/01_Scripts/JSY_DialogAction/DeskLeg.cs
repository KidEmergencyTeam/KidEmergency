...
public class DeskLeg : MonoBehaviour
{
    ...
    public void Holding()
    {
        ...
        bool isInteractable = Vector3.Distance(_legs[0].transform.position, _leftHand.transform.position) < 0.05f && isLeftGrapped &&
                                  Vector3.Distance(_legs[1].transform.position, _rightHand.transform.position) < 0.05f && isRightGrapped;
        if (isInteractable)
        { 
